...
There is picke file in the source: banner.p (searched online)
Pickle files are python serialization files.
Let's decode the file with python:

```py
import pickle
pickle_file = open('banner.p', 'r')
...
This looks like a building when left rotated and seen as a whole. I suspect we
need to go thru all the tuples and transform them:

  ('#', 5) → #####

Let's translate the array to ruby:
...
That page is jsut donation's page next one is zip:
